...
namespace Demo.Scripts.Runtime
{
    ...
    public class FPSMovement : NetworkBehaviour
    {
        ...
        private void UpdateMovementState()
        {
            ...
            // Save inputs in queue
            Pair movmentPair = new Pair();

            // X axis
            movmentPair.value = Input.GetAxisRaw("Horizontal");
            movmentPair.time = System.DateTime.Now.AddMilliseconds(latencyManager.playerLatency);
            latencyManager.movementXQueue.Enqueue(movmentPair);

            // Y axis
            movmentPair.value = Input.GetAxisRaw("Vertical");
            movmentPair.time = System.DateTime.Now.AddMilliseconds(latencyManager.playerLatency);
            latencyManager.movementYQueue.Enqueue(movmentPair);


            while (latencyManager.movementXQueue.Count > 0 && latencyManager.movementXQueue.First().time <= System.DateTime.Now)
            {
                _inputDirection.x = latencyManager.movementXQueue.Dequeue().value;
            }

            while (latencyManager.movementYQueue.Count > 0 && latencyManager.movementYQueue.First().time <= System.DateTime.Now)
            {
                _inputDirection.y = latencyManager.movementYQueue.Dequeue().value;
            }
